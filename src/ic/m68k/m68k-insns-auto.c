...
tme_m68k_fetch16(struct tme_m68k *ic, tme_uint32_t pc) 
{
  ...
  tme_uint32_t linear_address_last = linear_address_first + sizeof(tme_uint16_t) - 1;
  struct tme_m68k_tlb *tlb = tme_memory_atomic_pointer_read(struct tme_m68k_tlb *, ic->_tme_m68k_itlb, &ic->_tme_m68k_tlbs_rwlock);
  tme_uint16_t mem_value;
  const tme_shared tme_uint16_t *mem;
              ...
              unsigned int flags)
{
  ...
  for(; transferred < reg_size; ) {
    ...
    for (; __tme_predict_false(tme_bus_tlb_is_invalid(&tlb->tme_m68k_tlb_bus_tlb)
                               || (tlb->tme_m68k_tlb_function_codes_mask & TME_BIT(function_code)) == 0
                               || linear_address < tlb->tme_m68k_tlb_linear_first
                               || linear_address > tlb->tme_m68k_tlb_linear_last
    ...
    if (__tme_predict_true(mem != TME_EMULATOR_OFF_UNDEF)) {
      ...
      if (__tme_predict_false((cycle_size - 1)
                              > (tlb->tme_m68k_tlb_linear_last - linear_address))) {
        cycle_size = (tlb->tme_m68k_tlb_linear_last - linear_address) + 1;
      }
